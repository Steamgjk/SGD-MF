...
std::vector<int>visitingFlag;
std::vector<int>updateTimes;
queue<int> seq_qu;
double P[N][K];
double Q[K][M];
...
int main(int argc, const char * argv[])
{
    ...
    visitingFlag.resize(P * P);
    updateTimes.resize(P * P);
    for (int i = 0 ; i < P; i++)
    {
        for (int j = 0 ; j < P; j++)
        {
            visitingFlag[i * P + j] = 0;
            updateTimes[i * P + j] = 0;
        }
    }
    ...
    partitionP(P, Pblocks);
    partitionQ(P, Qblocks);
    srand(time(0));
    bool ret = false;
    ...
    for (int i = 0; i < WORKER_NUM; i++)
    {
        block_id = ChooseBlock();
        worker_pidx[i] = block_id / P;
        worker_qidx[i] = block_id / Q;
        //update flag and updatesTimes
        visitingFlag[block_id] = 1;
        updateTimes[block_id]++;
    }
    for (int i = 0; i < WORKER_NUM; i++)
    {
...
int ChooseBlock()
{
    std::vector<int> tmp = visitingFlag;
    std::vector<int> tmp2;
    int min_val = 999999;
    for (int i = 0; i < P; i++)
    {
        for (int j = 0; j < P; j++)
        {
            ...
            {
                if (min_val > updateTimes[i * P + j])
                {
                    min_val = updateTimes[i * P + j];
                }
            }
        }
    }
    for (int i = 0; i < P; i++)
    {
        for (int j = 0; j < P; j++)
        {
            if (tmp[i * P + j] == 0 && min_val == updateTimes[i * P + j])
            {
                tmp2.push_back(i * P + j);
            }

        }
    }
    int r = rand() % (tmp2.size());
    return tmp2[r];

}
void LoadSeqConfig()
{
    ifstream ifs(SEQ_NAME);
    int num;
    ifs >> num;
    int block_id;
    for (int i = 0; i < num; i++)
