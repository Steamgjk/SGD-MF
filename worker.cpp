        ...
        if (StartCalcUpdt[td_id])
        {
            ...
            while (times_thresh--)
            {
                ...
                for (int k = 0; k < K; ++k)
                {
                    ...
                    {
                        printf("p %d q %d  error =%lf i=%d j=%d k=%d rand_idx=%d vale=%lf\n", p_block_idx, q_block_idx, error, i, j, k, rand_idx,  rates_for_col_threads[p_block_idx][q_block_idx][td_id][rand_idx] );
                        getchar();
                    }
                }
